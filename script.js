...
window.addEventListener("DOMContentLoaded", function () {
  ...
  listCon.innerHTML = ""; // Clear existing list

  // Iterate through stored todos and add them to the list
...
//toggles the text color in the nav-container
function allToggle() {
...
function removeTodoFromStorage(task) {
  const storedTodos = JSON.parse(localStorage.getItem("todos")) || [];
  const updatedTodos = storedTodos.filter((todo) => todo.task !== task);
  localStorage.setItem("todos", JSON.stringify(updatedTodos));
}

// Update the cross function to remove todos from local storage
function cross() {
  const cross = document.querySelectorAll(".cross-icon");
  cross.forEach(function (remove) {
    remove.addEventListener("click", function () {
      const div = remove.parentNode;
      const listItem = div.querySelector("li");
      const task = listItem.textContent;

      // Remove todo from local storage
      removeTodoFromStorage(task);
